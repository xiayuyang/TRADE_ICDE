...
def run_mtmc(cams_dir = 'datasets/AIC22_Track1_MTMC_Tracking/train/S10',
            ...
            for tid in mot_list:
                ...
                if ((frame_idx - frame_list[-1] > 20) and (tid not in trackers_avg_feat)):
                    ...
                    if start_zone: # 起始区域为0，则必为新轨迹
                        matched_tid = cross_cam_match(cam, start_zone, io_time, mean_feat, tid, lazy_threshold)
                        if end_zone:
                    ...
                    # for i in frame_list:                  
                    #     gt_detect[cam][cur_gtid][i] = [int(mot_list[tid][i]['bbox'][0]),int(mot_list[tid][i]['bbox'][1]),
    ...
    # pickle.dump(trackers_avg_feat,open('test_tracklet.pkl','wb'))

    # 分层聚类
    # t16 = time_synchronized()
    # cid_tid_dict = dict()
    # for k, _ in trackers_avg_feat.items():
    #     tracklet = trackers_avg_feat[k]
    #     tid = tracklet['tid']
    #     cid = int(tracklet['cam'][-1:])
    #     if (cid, tid) not in cid_tid_dict:
    #         cid_tid_dict[(cid, tid)] = tracklet
    # scene_cluster = [1, 2, 3, 4, 5]
    # cid_tids = sorted([key for key in cid_tid_dict.keys() if key[0] in scene_cluster])
    # clu = get_labels(cfg,cid_tid_dict,cid_tids,score_thr=0.1)
    ...
    #     for c in c_list:
    #         cid_tid_label[c] = i + 1
    # t17 = time_synchronized()
    # total_cluster_time = (t17 - t16)
...
# def profile_multi(process_id, gpu_id, frame_qs, traj_qs, cams_dir_, weights_, imgsz_, cams_ratio_, lazy_threshold_):
#     print("传过来的参数:", cams_dir_, weights_, imgsz_, cams_ratio_, lazy_threshold_, gpu_id)
#     global GPU_ID
#     GPU_ID = gpu_id
#     mm_data, total_detect_time, total_extract_time, total_sct_time, total_pp_time, total_match_time=run_mtmc(cams_dir=cams_dir_, weights=weights_, imgsz=imgsz_, cams_ratio=cams_ratio_, lazy_threshold=lazy_threshold_)
...
#     result_json['total_match_time'] = total_match_time
#     return accuracy, processing_time, result_json
    # run_mtmc(cams_dir = 'datasets/AIC22_Track1_MTMC_Tracking/train/S10',
    #          weights = 'yolov5s.pt', 
    #          imgsz = 1280, 
    #          cams_ratio = [1, 1, 1, 1, 1],
    #          lazy_threshold = 0.7):
...
#         processing_time = total_detect_time+total_extract_time+total_sct_time+total_pp_time+total_match_time
#         cams = os.listdir('datasets/AIC22_Track1_MTMC_Tracking/train/S10')
#         cam_num = len(cams)
#         precision_all = 0 
#         recall_all = 0
#         f1_score_all = 0
