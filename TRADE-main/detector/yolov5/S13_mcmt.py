...
def cross_cam_match(cam, start_zone, io_time, new_feat, tid, trackers_avg_feat, gt_detect, max_similarity = 0.7):
    # param cam:当前轨迹的摄像头名
    ...
    max_range = max(closest_idx, match_list_size - closest_idx)
    # NOTE 可能需要限制一下匹配的idx大小，因为会跟时间差太大的轨迹匹配，可能是同车型的不同目标
    for search_idx in range(2 * max_range):
        cur_idx = (closest_idx + search_idx // 2) if search_idx % 2 == 0 else (closest_idx - 1 - search_idx // 2 )
        if (cur_idx >= 0 and cur_idx <= (match_list_size - 1) and io_time[0] < match_list[cur_idx][0]):
    ...
    if match_flag:       
        if match_list[match_idx][3]:
            ...
            # 还需要修改gt_detect
            gt_detect[cam][conflitct_gtid] = gt_detect[cam].pop(match_list[match_idx][1])

        return match_list[match_idx][1], match_flag
    else:
        return -1, match_flag     
...
def run_mtmc(cams_dir = 'datasets/AIC22_Track1_MTMC_Tracking/train/S13',
             weights = ['yolov5s.pt','yolov5s.pt','yolov5s.pt','yolov5s.pt'], 
             imgsz = [1280,1280,1280,1280], 
             cams_ratio = [1, 1, 1, 1],
             lazy_threshold = 0.7):
    # 加载模型
    ...
    trackers_avg_feat = {} # key是tid，value是该轨迹的相关信息
    datasets = {}            # 帧迭代器
    trackers = {}            # 追踪器
                    ...
                    if len(det):
                        ...
                        for *xyxy, conf, cls in reversed(det):
                            ...
                            if True:
                                ...
                                confidence_score.append(det_conf)
                            det_num += 1
                ...
                if ((frame_idx - frame_list[-1] > 20) and (tid not in trackers_avg_feat)):
                    ...
                    if start_zone: # 起始区域为0，则必为新轨迹
                        if start_zone != end_zone:
                            matched_tid, match_flag = cross_cam_match(cam, start_zone, io_time, mean_feat, tid, trackers_avg_feat, gt_detect)
                        if not match_flag:
                            g_tid += 1
                            matched_tid = g_tid
                        if end_zone:
                    ...
                    if matched_tid not in gt_detect[cam]:
                    ...
                    for i in frame_list:
                        ...
                        mm_data[cam][i]['detections'].append([matched_tid,x,y,w,h])   
                        gt_detect[cam][matched_tid][i] = [x,y,w,h]
        ...
        with open(gt_write, "w") as gt_file:
            for gid, v in gt_detect[cam].items():
                for fid, bbox in v.items():
                    gt_file.write(f'{fid},{gid},{bbox[0]},{bbox[1]},{bbox[2]},{bbox[3]},1,-1,-1,-1\n')
...
def profile(cams_dir_, weights_, imgsz_, cams_ratio_, lazy_threshold_, gpu_id):
    ...
    result_json['sim_cal_num'] = sim_cal_num
    confidence_score = np.array(confidence_score)
    np.save('confidence_score.npy', confidence_score)
    return accuracy, processing_time, result_json

# if __name__ == '__main__':
#     cams_dir_ = 'datasets/AIC22_Track1_MTMC_Tracking/train/S13'
#     cams_ratio_ = [1,1,1,1]
#     with torch.no_grad():
#         mm_data, total_detect_time, total_extract_time, total_sct_time, total_pp_time, total_match_time = run_mtmc()
...
#         precision, recall = summary['precision'][0], summary['recall'][0]
#         f1 = 2 * recall * precision / (precision + recall)
#         print(f'cam:{cam}, f1:{f1}, precision:{precision}, recall:{recall}')
#         ratio = (1.0 / cams_ratio_[i]) / total_ratio
#         precision_all += precision * ratio
#         recall_all += recall*ratio
#         f1_score_all += f1*ratio        
...
#     pre = precision_all / 1.0
#     rec = recall_all / 1.0
#     with open('S13/metrics.txt', "a") as result_file:
#         result_file.write(f'accuracy:{accuracy}, precision:{pre}, recall:{rec}\n')
